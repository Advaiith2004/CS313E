...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for i in range(len(feedback_colors)):
            if feedback_colors[i] == CORRECT_COLOR:
                ...
                if self.colors[guessed_word[i]] == NO_COLOR:
    ...
    def __str__(self):
        ...
        """
        out_put = ""
        for word in range(len(self.rows[0])):
            if word != len(self.rows[0]) -1:
                out_put += f"{color_word(self.colors[self.rows[0][word]], self.rows[0][word])} "
            else:
                out_put += f"{color_word(self.colors[self.rows[0][word]], self.rows[0][word])}"
        out_put += "\n" + " "
        for letter in range(len(self.rows[1])):
            if letter != len(self.rows[1]) -1:
                out_put += f"{color_word(self.colors[self.rows[1][letter]], self.rows[1][letter])} "
            else:
                out_put += f"{color_word(self.colors[self.rows[1][letter]], self.rows[1][letter])}"
        out_put += "\n" + "   "
        for alpha in range(len(self.rows[2])):
            if alpha != len(self.rows[2]) -1:
                out_put += f"{color_word(self.colors[self.rows[2][alpha]], self.rows[2][alpha])} "
            else:
                out_put += f"{color_word(self.colors[self.rows[2][alpha]], self.rows[2][alpha])}"
        return out_put
...
class WordFamily:
    ...
    def __init__(self, feedback_colors, words):
        ...
        self.words = words
        self.difficulty = 0
        for word in self.feedback_colors:
            if word is WRONG_SPOT_COLOR:
                self.difficulty += 1
            elif word is NOT_IN_WORD_COLOR:
                self.difficulty += 2
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if secret_word[i] == guessed_word[i]:
            ...
            secret_list.remove(secret_word[i])

    for j in range(NUM_LETTERS):
        if secret_word[j] != guessed_word[j] and guessed_word[j] in secret_list \
        and feedback[j] is None:
            feedback[j] = WRONG_SPOT_COLOR
            secret_list.remove(guessed_word[j])
        elif feedback[j] is None:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for word in remaining_secret_words:
        ...
        if feedback_key not in word_family:
    ...
    for i in word_family.keys():
        if len(word_family[i]) > max:
            ...
            word_track = word_family[i]
            key = i
            continue
        elif len(word_family[i]) == max:
            word_track = None
            key = None

    if word_track is not None:
        new_remaining_words = word_track
        feedback_colors = key
    else:
        for j in word_family.keys():
            diff =  0
            for k in j:
                if k == WRONG_SPOT_COLOR:
                    diff += 1
